...
export default async function ProgressPage({
  ...
  if (!accessToken) {
    return (
      <div className="p-4 sm:p-6">
        <h1 className="text-lg sm:text-xl font-semibold">Falta token de acceso</h1>
  ...
  if (!courseId) {
    return (
      <div className="max-w-5xl mx-auto p-4 sm:p-6">
        <h1 className="text-xl sm:text-2xl font-bold mb-4">Progreso</h1>
        <p className="text-muted-foreground text-sm sm:text-base">Error: ID del curso no válido.</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      <div className="flex items-start justify-between flex-col sm:flex-row gap-4">
        <div className="flex-1">
          ...
          <p className="text-muted-foreground mt-1 text-sm sm:text-base">{currentCourse?.name}</p>
        </div>
